...
function Input.new(modules)
  ...
  input.update = function()
    ...
    local active_bits = 0
    if bit32.band(io[1][io.ports.JOYP], 0x20) == 0 then
      active_bits = bit32.bor(active_bits, button_bits)
    end
    if bit32.band(io[1][io.ports.JOYP], 0x10) == 0 then
      active_bits = bit32.bor(active_bits, d_pad_bits)
    end
    active_bits = bit32.bnot(active_bits)

    io[1][io.ports.JOYP] = bit32.bor(0xC0, bit32.band(io[1][io.ports.JOYP], 0x30), bit32.band(active_bits, 0x0F))
  end
  ...
  io.write_logic[io.ports.JOYP] = function(byte)
    io[1][io.ports.JOYP] = bit32.band(byte, 0x30)
    input.update()
